    ...
    <section id="table-success">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Health Facilities List</h4>
            </div>
            <!-- datatable start -->
            <div class="table-responsive">
                <div id="table-extended-success_wrapper" class="dataTables_wrapper no-footer"><table id="table-extended-success" class="table mb-0 dataTable no-footer" role="grid">
                        ...
                        <tr role="row">
                            ...
                            <th class="sorting_disabled" rowspan="1" colspan="1" aria-label="account details" style="width: 228px;">Facility name</th>
                        ...
                        @if($facilities)
                            @foreach($facilities as $facility)
                                <tr role="row" class="odd">
                                    ...
                                    <td class="text-bold-600">{{$facility->id}}</td>
                                    <td>{{$facility->name}}</td>
                                    <td class="text-success">{{$facility->created_at->diffForHumans()}}</td>
                                    <td class="text-success">{{$facility->updated_at->diffForHumans()}}</td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right" x-placement="bottom-end" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(19px, 19px, 0px);">
                                                <a class="dropdown-item" href="{{url('/edit_facility/'.$facility->id)}}"><i class="bx bx-edit-alt mr-1"></i> edit</a>
                                                <a class="dropdown-item" href="{{url('/delete_facility/'.$facility->id)}}"><i class="bx bx-trash mr-1"></i> delete</a>
                                            </div>
                                        </div>
                                    </td>
